...
extern Nodo_t *lista, *listaScheduled, *listaTagMancanti, listaTagMancantiPrevious;
...
int modifica=0, statoDisplay=0, statoDisplayP=0;
...
void assegna_medicinale(medicinale *dest, medicinale *sur){
	...
	dest->minA = sur->minA;
	dest->oraA = sur->oraA;
	dest->giorniAllaScadenza = sur->giorniAllaScadenza;
...
extern int fIntervalReached;
extern uint32_t previousSec;
...
extern uint8_t bUid[10], bUidPresent[100], nCardDetected, nCardPrevious, nCardTot;
...
//        				vTaskDelay(5000);
//        				if (nCardDetected<nCardTot) {
//        					GUI_DispStringAt( "Riposiziona la medicina", 0, 150);
//        				}
//        				fAlarmTimeMatched=0;
//        				modifica=1;
//        				//MODIFICARE DOSI RIMANENTI
...
//M	        vCardDisplay( );
	...
	while ( 1 ) {
		...
		if (statoDisplay==statoDisplayP) {
			...
			case 2  :
				GUI_Clear();
				//GUI_DispStringAt( "Stato 2 - manca", 120-7*10/2, 50);
				mostraElencoMancanti();
				showTime();
				break; /* optional */

			case 3  :
				GUI_Clear();
				GUI_DispStringAt( "Stato 3 - tag corretto", 120-7*10/2, 50);
				showTime();
				break; /* optional */

			case 4  :
				GUI_Clear();
				GUI_DispStringAt( "Stato 4 - rimetti tag", 120-7*10/2, 50);
				showTime();
				break; /* optional */

			case 5  :
				GUI_Clear();
				GUI_DispStringAt( "Stato 5 - tag errato", 120-7*10/2, 50);
				showTime();
				break; /* optional */

			case 6  :
				GUI_Clear();
				//GUI_DispStringAt( "Stato 6 - allarme", 120-7*10/2, 50);
				showTimeAlarm(bUidPresent,nCardDetected,nCardPrevious);
				showTime();
				break; /* optional */
			...
			default : /* Optional */
				GUI_Clear();
				GUI_DispStringAt( "Stato DEFAULT", 120-7*10/2, 50);
				showTime();
			}
		}
		statoDisplayP = statoDisplay;
